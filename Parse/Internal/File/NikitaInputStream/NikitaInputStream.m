...
@interface NikitaInputStream() <NSStreamDelegate>
@end

@implementation NikitaInputStream {
    NSString *_path;
...
- (BOOL)respondsToSelector:(SEL)aSelector {
    IMP implementation = method_getImplementation(class_getInstanceMethod([self class], aSelector));
    if (implementation == NULL || implementation == _objc_msgForward || implementation == _objc_msgForward_stret) {
        return [_inputStream respondsToSelector:aSelector];
    }
    return YES;
}
...
- (NSInteger)read:(uint8_t *)buffer maxLength:(NSUInteger)len {
    if (!_finished) {
        long currentOffset = [[_inputStream propertyForKey:NSStreamFileCurrentOffsetKey] longValue];
        long fileSize = lseek(_fd, 0, SEEK_END);

        len = MIN(len, ((fileSize - currentOffset) - 1));
    }
